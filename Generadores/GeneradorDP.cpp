...
void genTrans() {
    ...
    for (int i = long_min; i <= long_max; i += incremento) {
        ...
        escribir("caso3DP.txt", S1_transposicion, S2_transposicion);
    }
}

void genDistintos() {
    ofstream archivo("caso4DP.txt");
    for (int i = long_min; i <= long_max; i += incremento) {
        string S1_diferentes = generarCadena(i, 'a');
        string S2_diferentes = generarCadena(i, 'z');
        escribir("caso4DP.txt", S1_diferentes, S2_diferentes);
    }
}

void genSimilar() {
    ofstream archivo("caso5DP.txt");
    for (int i = long_min; i <= long_max; i += incremento) {
        ...
        escribir("caso5DP.txt", S1_similares, S2_similares);
    }
}

void genLongDistinto() {
    ofstream archivo("caso6DP.txt");
    for (int i = long_min; i <= long_max; i += incremento) {
        string S1 = genAleatorio(i);
        string S2 = genAleatorio(i + 2);
        escribir("caso6DP.txt", S1, S2);
    }
}

void genAleatorio() {
    ofstream archivo("caso7DP.txt");
    for (int i = long_min; i <= long_max; i += incremento) {
        string S1_aleatorio = genAleatorio(i);
        string S2_aleatorio = genAleatorio(i);
        escribir("caso7DP.txt", S1_aleatorio, S2_aleatorio);
    }
}
